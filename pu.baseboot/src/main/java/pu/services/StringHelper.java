...
public static String align( String aValue, int aMaxLen, int aAlignment )
{
	...
	switch( aAlignment )
	{
		...
		default:
			throw new RuntimeException( "Invalid alignment: " + aAlignment );
	}
}
/**
...
public static List<String> classPathAsList()
{
	...
	List<String> classPathList = StringHelper.explode2ToList( classPath, FileHelper.getPathSeparator() );
...
public static List<String> converteerTekst( String s, int aRegelLengte )
{
	...
	for ( Iterator<String> it = explode2ToList( ss, ENTER ).iterator(); it.hasNext(); )
	{
		res.addAll( verdeelInRegelsMetVasteKantlijn( it.next(), aRegelLengte ) ); 
	}
	return res;
}
/**
 ...
 * @param separator The string to be inserted between the elements
 */
public static String implode( Collection<String> aCollection, String aSeparator )
{
	if ( aCollection == null )
	{
...
public static String quoteXmlChars( String s )
{
    ...
    String retS = s;
    retS = StringHelper.replaceAll( retS, "&", "&amp;" );
    retS = StringHelper.replaceAll( retS, "<", "&lt;" );
    retS = StringHelper.replaceAll( retS, ">", "&gt;" );
    return retS;
}
...
public static String showDiff( String lString, String rString )
{
	...
	if ( lString.length() != rString.length() )
	{
		...
		sb.append( FileHelper.getLineSeparator() );
	}
...
public static List<String> verdeelInRegelsMetVasteKantlijn( String s, int regelLengte )
{
	...
	if ( chars.length - oudePos > regelLengte )
	{
		throw new RuntimeException( "Laatste regel groter dan regellengte" );
	}
