 ...
 */
private Hashtable<TreeNode, DBItem> dataHTree = new Hashtable<>();
...
/**
  Command stack
 */
DefaultListModel<String> cmdstack;
...
public DBItem(int aTypeOfItem, String aSchema, String aName){
	typeOfItem = aTypeOfItem;
	schema = aSchema;
	item_name = aName;
	switch(aTypeOfItem){
		case 0:
			...
			break;
		case 1:
			sql = "select * from "+aSchema + "." + item_name;
	}
}
}
...
public JDBCQueryTool(QueryAgent in_query_agent) {
	...
	auto_resize_result_table.addActionListener(evt ->
	{
	...
	auto_commit.addActionListener(evt ->
	{
		try{
	...
	paste_table.addActionListener(evt -> tablenameBox());
	...
	dbTree.addTreeSelectionListener(e -> evndbTreeSelectionChanged(e));
	...
	)
	{
		...
		while( listofSchema.next() )
		{
			...
			try ( ResultSet listofTables = query_agent.executeQuery(new Query("show tables")); )
			{
	...
	commandlist.addListSelectionListener( lse ->
	{
		if (lse.getValueIsAdjusting()) {
			String selectedcmd;
			selectedcmd = commandlist.getSelectedValue();
			qtext = selectedcmd;
	...
	stop.addActionListener(evt -> query_agent.cancelQuery());
...
private void evndbTreeSelectionChanged(TreeSelectionEvent e) {
	...
	{
		query_text_area.setText(dataHTree.get(selectednode).sql);
	}
}
...
public static void main(String[] args) {
	...
	if (username == null) {
		...
		try {
			...
			SwingUtilities.invokeLater(( ) -> createNewWindow());
...
private void tablenameBox() {
	String schemaname;
	DefaultListModel<String> tablenames;
	JPanel jp;
	...
	try {
		...
		paste.addActionListener( e ->
		{
			...
			qtext = tablelist.getSelectedValue();
			query_text_area.replaceSelection( qtext );
		});

		JButton cols = new JButton( "Columns");
		cols.addActionListener( e -> columnnameBox( tablelist.getSelectedValue() ));
		...
		try ( PrintWriter ps = new PrintWriter( new FileOutputStream( "test.log", true ), true ); )
		{
 ...
 */
private DefaultListModel<String> getTableNames( String schemaname ) {
	ResultSet listofTables;
	DefaultListModel<String> tnames;
...
private void columnnameBox(String tablename) {
	DefaultListModel<String> columnnames;
	JPanel jp;
	...
	paste.addActionListener( e ->
	{
		...
		qtext = columnlist.getSelectedValue();
 ...
 */
private DefaultListModel<String> getColumnNames( String tablename ) {
	ResultSet lcols;
	DefaultListModel<String> tcols;
